...
class Client(BaseClient):
    ...
    def __init__(
        ...
        super().__init__(
            proxy=proxy,
            verify=verify,
            base_url=HTTPS_BASE_URL if use_https else BASE_URL,
        )
        self.fetch_interval_hours = fetch_interval_hours
        if DBotScoreReliability.is_valid_type(reliability):
            self.reliability = DBotScoreReliability.get_dbot_score_reliability_from_str(
                reliability
            )
        else:
            return_error(
                "PhishTankV2 error: Please provide a valid value for the Source Reliability parameter."
            )
...
def test_module(client: Client) -> str:
    ...
    if response_was_empty:
        return_error(
            "Error - could not fetch PhishTankV2 database, "
...
def is_phishtank_data_outdated(client: Client, context: dict):
    ...
    return context["timestamp"] < date_to_timestamp(
        current_time - fetch_interval_seconds
    )
...
def is_reload_needed(client: Client, context: dict) -> bool:
    ...
    Returns: True if DB can be loaded now. i.e DB was not loaded in the last fetch_interval hours.
            False otherwise.
    """
    return not was_phishtank_data_ever_reloaded(context) or is_phishtank_data_outdated(
        client, context
    )
...
def create_verified_markdown(url_data: dict, url: str):
    ...
    phish_tank_url = (
        f'http://www.phishtank.com/phish_detail.php?phish_id={url_data["phish_id"]}'
    )
    ...
    for url in url_list:
        ...
        if url_data_is_valid:
            ...
            dbot = Common.DBotScore(
                url, DBotScoreType.URL, "PhishTankV2", 0, "", client.reliability
            )
...
def phishtank_reload_command(client: Client):
    ...
    last_load = datetime.utcfromtimestamp(context["timestamp"] / 1000.0).strftime(
        "%a %b %d %Y %H:%M:%S (UTC)"
    )
    output_to_context = {"value": last_load}
    return CommandResults(
        readable_output=readable_output,
        outputs=output_to_context,
...
def phishtank_status_command():
    ...
    if data_was_not_reloaded_yet:
        ...
        last_load = datetime.utcfromtimestamp(data["timestamp"] / 1000.0).strftime(
            "%a %b %d %Y %H:%M:%S (UTC)"
        )
        number_of_urls_loaded = len(data["list"].keys())
        status += (
            f"Total **{number_of_urls_loaded}** URLs loaded.\n"
...
def main() -> None:
    ...
    if not is_number(fetch_interval_hours):
        return_error(
            "PhishTankV2 error: Please provide a numeric value (and bigger than 0) for Database refresh "
            "interval (hours)"
        )
